...
def _event(db_path: str, level: str, message: str, data: Optional[Dict[str, Any]] = None,
           run_id: Optional[int] = None, task_id: Optional[int] = None):
    conn = sqlite3.connect(db_path)
    try:
        conn.execute(
            "INSERT INTO events(run_id, task_id, level, message, data_json, created_at) VALUES(?,?,?,?,?,?)",
            (run_id, task_id, level, message, json.dumps(data or {}, ensure_ascii=False), datetime.utcnow().isoformat())
        )
        conn.commit()
    finally:
        conn.close()

def _mem_search(query: str, top_k: int, rag_tag: Optional[str], min_score: float) -> List[Dict[str, Any]]:
    """Llama al CLI oficial para obtener contexto (RAG). Retorna lista de resultados."""
    args = [sys.executable, "-m", "wilbito.interfaces.cli", "mem-search", query, "--top-k", str(top_k)]
    if rag_tag:
