...
def load_cfg() -> dict:
    import yaml
    with open(os.path.join(os.path.dirname(__file__), "..", "config", "base.yaml"), "r", encoding="utf-8") as f:
        cfg = yaml.safe_load(f)
    return cfg

def _resolve_objetivo(objetivo_arg: Optional[str], objetivo_opt: Optional[str]) -> str:
    if objetivo_opt:
        return objetivo_opt
    if objetivo_arg:
        return objetivo_arg
...
@app.command("pr")
def pr_review(
    objetivo: Optional[str] = typer.Argument(None),
    objetivo_opt: Optional[str] = typer.Option(None, "--objetivo", "-o"),
):
